...
*
*  Repository contains the public methods:
*  ------------------------------------------
...
*  - getMessage(): gets the message from Comm
*  - checkFileSendStatus(): sends the message with the final sent status
...
#include "../Project2/NoSQLDB/Executive/NoSqlDb.h"
...
namespace Repository
{
	...
	//-------< sends the message with the final sent status  >---------------------------

	inline void Server::checkFileSendStatus(MsgPassingCommunication::Message msg) {
		if (msg.containsKey("file")) {
			msg.remove("file");
			msg.attribute("content", "File Received Successfully");
			msg.command("File_Sent");
			postMessage(msg);
		}
	}

...
const std::string clientRecevingPath = "../../../Client";
...
void Sender::start()
{
  std::function <void()> threadProc = [&]() {
    while (true)
    {
      ...
      if (msg.to().address != lastEP.address || msg.to().port != lastEP.port || msg.command() == "connectServer")
      {
        connecter.shutDown();
        //connecter.close();
...
bool Sender::sendFile(Message msg)
{
  ...
  if (msg.value("sender") == "server") {
	  fileSpec = sendFilePath + "/" + msg.file();
  }
  else if (msg.value("sender") == "client") {
	 fileSpec = msg.value("path");
  }
...
public:
  ...
  */
  bool receiveFile(Message msg)
  {
	...
	// server to client
	if (msg.value("sender") == "server") {
		fileSpec = clientRecevingPath + "/" + msg.value("filename");
	}
	else if (msg.value("sender") == "client") {
...
void DemoSndrRcvr(const std::string& machineName)
{
  ...
  SocketSystem ss;  EndPoint ep1;  ep1.port = 9091;
  ep1.address = "localhost";  Receiver rcvr1(ep1);
  BlockingQueue<Message>* pQ1 = rcvr1.queue();
  ClientHandler ch1(pQ1);  rcvr1.start(ch1);
  EndPoint ep2;  ep2.port = 9092; ep2.address = "localhost";  Receiver rcvr2(ep2);
  BlockingQueue<Message>* pQ2 = rcvr2.queue();
  ClientHandler ch2(pQ2);  rcvr2.start(ch2);
  Sender sndr;  sndr.start();  bool connected = sndr.connect(ep1);
  Message msg;  msg.name("msg #1");  msg.to(ep1);
  msg.from(msg.to());  msg.command("do it");
  msg.attribute("bodyAttrib", "zzz");  StaticLogger<1>::flush();
  std::cout << "\n  sndr in main posting message:  " << msg.name();
  sndr.postMessage(msg);

  msg.name("msg #2");  msg.to(EndPoint(machineName, 9092));  StaticLogger<1>::flush();  std::cout << "\n  sndr in main posting message:  " << msg.name();
  sndr.postMessage(msg);
  ...
  StaticLogger<1>::flush();  rcvdMsg = rcvr1.getMessage();  std::cout << "\n  rcvr1 in main received message: " << rcvdMsg.name();
  rcvdMsg.show();  rcvr1.stop();  rcvr2.stop();  sndr.stop();  StaticLogger<1>::flush();
...
{
  SUtils::title("Demonstrating Comm class");  SocketSystem ss;
  ...
  comm1.stop();  comm2.stop();  StaticLogger<1>::flush();  std::cout << "\n  press enter to quit DemoComm";  _getche();
}
/////////////////////////////////////////////////////////////////////
